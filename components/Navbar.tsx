  ...
  return (
    <nav 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 px-8 ${
        isScrolled || mobileMenuOpen ? 'bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-200/50' : 'bg-transparent'
      }`}
    >
        ...
        <div className="hidden md:flex space-x-6">
          {NAV_LINKS.map((link) => (
            <a 
              ...
              className={`text-sm transition-all duration-300 py-1 relative group cursor-pointer ${
            ...
            >
              ...
              <span className={`absolute -bottom-1 left-0 w-full h-[2px] bg-black transition-transform duration-300 origin-left ${
                activeSection === link.href ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-50'
              }`} />
            </a>
          ))}
        </div>
